...
LCD_DAT   equ 8001h ; Set this to the address of the data register
...
wdat:        mov        EMI0CN,#HIGH LCD_CMD ; command/status register
  mov r1,#LOW LCD_CMD
wdat1:        movx        a,@r1                ; get status register
        jb        acc.7,wdat1        ; wait for not BUSY
        inc        r1; point to data register
        mov        a,r0                ; actual data to write
        movx        @r1,a                ; write the data
        ret
;
...
init_lcd:
        ...
        mov        emi0cf,#38h     ; B5: P4-7, B4: non-muxed, B3-2 split bank
...
blank_loop:
        movx        @dptr, a
        inc        dptr
        rl a               ; Rotate left to create pattern: 1?2?4?8?...
        mov        b, dph
        jnb        b.2, blank_loop
        ret
